...
def scrape_profile_page(page):
    ...
    phone_val = None
    try:
        phone_locator = page.locator("span.webcms-phone").locator("xpath=..")
        if phone_locator.count() > 0:
            full_phone_text = phone_locator.inner_text(timeout=2000)
            if "Office" in full_phone_text:
                phone_val = full_phone_text.split("Office")[1].strip()
    except TimeoutError:
        logging.warning(f"Could not find a phone number for {name}.")
