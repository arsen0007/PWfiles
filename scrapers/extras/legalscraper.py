...
def get_total_entries(page, city) -> int | None:
    ...
    try:
        ...
        page.select_option(CITY_DROPDOWN, city)  # <-- This was the missing line
...
def main(city, max_profiles, profiles_per_page=10, retries=2):
    """
    Main function using the reliable but slow method of re-navigating for each profile.
    """
    scraped_data = []
    ...
    with sync_playwright() as p:
        ...
        if max_profiles == -1:
            logging.info("Scrape all mode enabled. Detecting total profiles...")
            temp_page = browser.new_page()
            # Pass the city to the counting function
            total_on_site = get_total_entries(temp_page, city)
            temp_page.close()
            if total_on_site:
                max_profiles = total_on_site
            else:
                logging.critical("Could not determine total profiles. Aborting.")
                browser.close()
                return

        logging.info(f"Starting scrape for city='{city}' with a target of {max_profiles} profiles.")
            ...
            for i in range(max_profiles):
                for attempt in range(retries):
                    try:
                        ...
                        logging.info(f"--- Preparing to scrape profile {i+1}/{max_profiles} (Page {target_page + 1}, Item {index_on_page + 1}) ---")
